...
1. Поднять PostgreSQL (например `docker compose up -d postgres` или свой инстанс). В `Server/appsettings.json` (или через переменные окружения) указать хост, порт, БД, пользователь и пароль.
...
- `Server/` — хост, Worker, протокол, сервисы, БД (EF Core, PostgreSQL), файловое хранилище
- `Client/` — консольный клиент, подключение к серверу, команды чата и файлов
- `Shared/` — общие модели и DTO
- В корне: `docker-compose.yml`, `.env.example`, `README.md`, `.dockerignore`
